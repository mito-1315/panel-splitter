...
export async function clearAllPanels(req, res) {
  try {
    await deleteAllRows();
    res.json({ message: 'All panels cleared successfully' });
  } catch (error) {
...
export async function bulkCreatePanels(req, res) {
  try {
    ...
    for (const panel of panels) {
      const { id, duration, time, panel: panelNumber, teamsDataId, teamName, teamId, problemStatementId, theme } = panel;
      
      try {
        await savePanelData(id, duration, time, panelNumber, teamsDataId, teamName, teamId, problemStatementId, theme);
        savedPanels.push(panel);
      } catch (error) {
        console.error(`Error saving panel ${id}:`, error);
      }
    }

    res.json({ 
      message: `Successfully created ${savedPanels.length} panels`, 
      count: savedPanels.length 
    });
  } catch (error) {
    console.error('Error in bulk panel creation:', error);
    res.status(500).json({ error: error.message });
  }
}
